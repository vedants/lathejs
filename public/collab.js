...
function init() {
  ...
  document.getElementById("draw").onclick = onDraw;
  document.getElementById("move").onclick = onMove;
...
function initLights() {
  //TODO: maybe if I parented the lights to the woodblock, they'd always be uniformly lit. 
...
function initObjects() {
  ...
  lathe.material = new THREE.MeshLambertMaterial( { color : 0x00bbbb} );
...
function check_and_cut(newSegmentFactors) {
  for (var i = 0; i < newSegmentFactors.length; i++) {
    if (newSegmentFactors[i] != segmentFactors[i] && newSegmentFactors[i] > lathe.minRadius) {
...
function onClick () {
  ...
  // Convert the pose orientation and position into THREE.Quaternion and THREE.Vector3 respectively
  var ori = new THREE.Quaternion( pose.orientation[0],pose.orientation[1],pose.orientation[2],pose.orientation[3]);
