...
var STROKE_WIDTH_MINIMUM = 0.0008;//original: 0.00125
var STROKE_WIDTH_MAXIMUM = 0.002; //original: 0.1
var STROKE_WIDTH_MODIFIER = 0.125; //original : 1.25
...
var MINIMIM_ERASE_ENERGY = 1.25; //original 0.005
...
THREE.ARUtils.getARDisplay().then(function (display) {
  if (display) {
    vrDisplay = display;
    //!!!
    //don't init anything because the renderer etc. is set up in collab.js. this file only contains helper functions. 
    //update loop is kicked off in init() as well, so that never gets called. 
    //init();
  } else {
...
function init() {
  ...
  ws = new WebSocket('ws://turnbywire.glitch.me'); //websocket server
  setUpWebSocket();
...
function processDraw() {
  ...
  if (strokes[strokeIndex]) {
      strokes_group.remove(strokes[strokeIndex]);
  }
  ...
  attach(strokes[strokeIndex], strokes_group, scene);
  
}

function updateStrokePositions(){
  for (var i = 0; i < strokes.length; i++) {
    var currStroke = strokes[i];
 ...
 */
var geometry = new THREE.SphereGeometry( 0.001, 8, 8);
var material = new THREE.MeshDepthMaterial();//new THREE.MeshBasicMaterial( {color: 0xffff00} );
...
function processDrawBrush() {
  ...
  getDrawPoint(reticlePos);
  reticle.position.copy(reticlePos); 
  scene.add(reticle);

  //old version, that shows the angle and face etc of the new stroke
...
function clearStrokes()
{
  ...
  for( var i = 0; i < len; i++ ) {
    strokes_group.remove(strokes[i]);
  }
}
/**
...
function clearLastStroke()
{
  ...
  //update strokes over the network 
  sendStrokesOverWebSocket();
}
/**
...
function sendStrokesOverWebSocket() {
  console.log(strokes);
  var strokes_json = JSON.stringify(strokes_group.children); 
