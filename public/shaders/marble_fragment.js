...
void main()
{
    ...
    for ( int i = 0; i < MAX_DIR_LIGHTS; i++) {
      ...
      float gaussian = (1.0/sqrt(2.0*PI*sigma)) * pow(E,pwr);
      
      //gl_FragColor = vec4(gaussian,0,0,1.0);
      ...
      float weight = 0.0;
      float slope = 1.0/10.0;
      
      float sign = 1.0;
      
      //works until size 12, then gets noticeable artifacts
      for (int i = 1; i < 10; i++)
      {
            float s = pow(2.0,float(i));
            float v1 = z*s;//+ rand(vec2(offset, offset));
            float v2 = y*s;
            float v3 = x*s;
            noise += (smoothNoise(v1,v2,v3,s)) * (weight + slope * float(i)) * (pow(2.0,10.0)/s);
            //noise += (smoothNoise(v3,0.0,s)) / 15.0;
            offset += 1.0;
            //sign = -1.0*sign;  
      }
      
      float divide = (1.0 - pow(2.0,7.0))/(1.0 - 2.0);
      ...
      float turbPower = 10.0;

      float value = x * xPeriod + y * yPeriod + z * zPeriod + turbPower * noise;//turbPower * turbulence(y,z,turbSize);
      float sineValue = abs(sin(value)) + gaussian;
      
      //gl_FragColor = vec4(noise,0,0,1.0);
      gl_FragColor = vec4(sineValue, sineValue, sineValue - 0.3,1.0);
...
//       float lightMult = 3.0 * length(directionalLightDirection[i]);
    ...
    */
    ...
    //gl_FragColor = vec4( abs(latheRadius / (thicknessPerp * 5.0)), 0.0,0.0,1.0);//vec4(DiffuseColour * ( totalDiffuse * DiffuseColour) + totalSpecular + ambientLightColor * ambient,1.0);//DiffuseColour * ( totalDiffuse * DiffuseColour) + totalSpecular + ambientLightColor * ambient,1.0);
